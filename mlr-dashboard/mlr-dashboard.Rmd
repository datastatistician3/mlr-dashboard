...
# 
#   model_variables <- c(input$xvar,input$yvar)
# 
...
#   # if(lvls<10|(class(df2$y) !='numeric'& class(df2$y) !='integer')){
...
#   # }
# 
#   # trainIndex <- caret::createDataPartition(df2$yvar,
#   #                                   p = 1-(testsize/100),
#   #                                   list = FALSE,
#   #                                   times = 1)
...
selectizeInput('slt_Tune','Parameter Tuning',choices = c('Coarse auto-tune (fast)','Fine auto-tune (slow)','manual'))
...
```{r spaghetti, echo=echo_chunks, message=message_chunks, results='asis', fig.height=8, fig.width=20}
  ...
  # bsModal('data1',title = 'Dataset1',trigger = 'btn_viewData',size = 'large',renderDataTable({rawdata()}))
# renderDataTable({
#   df2
# })
...
renderDataTable({
  ...
  # renderText({
  #   input$yvar
  #   unlist(stringr::str_split(input$xvar, pattern = " "))
  # })
  
  # trainIndex <- caret::createDataPartition(df$y,
  #                                   p = 1-(testsize/100),
  #                                   list = FALSE,
  #                                   times = 1)
  # dataTrain <<- df2[ trainIndex,]
...
# renderPlot({
#   hist(faithful$eruptions, probability = TRUE, breaks = as.numeric(input$sld_testsplit),
